...
def load_published_exp_data():
    ...
    depth_data = pd.read_csv('published_original/Fig5a_points.txt', names=['x','y'], delimiter=';')
    depth_data_err = pd.read_csv('published_original/Fig5a_err.txt', names=['x','y'], delimiter=';')
    depth = dict()
    ...
    radial_data_z600 = pd.read_csv('published_original/Fig5b_points_red.txt', names=['x','y'], delimiter=';')
    radial_data_z300 = pd.read_csv('published_original/Fig5b_points_blue.txt', names=['x','y'], delimiter=';')
    radial = dict()
...
def load_published_model_data():
    ...
    Spatial units all [um], light transmission decimals.
    """
    depth_curve = pd.read_csv('published_original/depth_profile.csv')
    depth = dict()
    ...
    radial_curve_z300 = np.load('published_original/BSF_model_radial_curve_300.npy')
    radial_curve_z600 = np.load('published_original/BSF_model_radial_curve_600.npy')
    radial = dict()
...
def load_matlab_model_data():
    matlab_data = loadmat('matlab_original/output3D_26_08_2024.mat')['out']
